...
def build_graphs(agent_model, prior_model, n_graphs_to_generate, batch_size, write_mols=False, mols_too=False,
    ...
    while n_generated_so_far < n_graphs_to_generate:
        ...
        if change_ap:
            with torch.no_grad():
                agent_apd = softmax(agent_model(nodes, edges))[1:]
        else:
            agent_apd = softmax(agent_model(nodes, edges))[1:]

        if change_ap:
            prior_apd = softmax(prior_model(nodes, edges))[1:]
        else:
            with torch.no_grad():
                prior_apd = softmax(prior_model(nodes, edges))[1:]
        nodes = nodes[1:]
    ...
    indexes = generated_agent_ll != 0
    generated_nodes = generated_nodes[indexes]
    generated_edges = generated_edges[indexes]
    generated_n_nodes = generated_n_nodes[indexes]
    ...
    if write_mols:
        ...
        graphs2 = [generated_nodes[:n_graphs_to_generate], generated_edges[:n_graphs_to_generate], generated_n_nodes[:n_graphs_to_generate]]
        graphs = [graphs1, graphs2]

        # remove extra zero padding from `generated_nlls` and `properly_terminated_graphs`
        generated_agent_ll = generated_agent_ll[:n_graphs_to_generate]
        generated_prior_ll = generated_prior_ll[:n_graphs_to_generate]
        
        properly_terminated_graphs = properly_terminated_graphs[:n_graphs_to_generate]
    ...
    return graphs, generated_agent_ll, generated_prior_ll, properly_terminated_graphs