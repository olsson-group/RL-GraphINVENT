...
def write_graphs_to_smi(smi_filename, molecular_graphs_list):
    ...
    with open(smi_filename, "w") as smi_file:
        ...
        for idx, molecular_graph in enumerate(molecular_graphs_list):
            ...
            try:
                ...
                smiles.append(rdkit.Chem.MolToSmiles(mol))
    ...
    return fraction_valid, validity_tensor, smiles
...
def write_model_status(epoch=None, lr=None, loss=None, score=None, append=True):
    ...
    if not append:  # create the file
        ...
        if C.job_type == "learn":  # only write a `convergence.csv` when training
            
            with open(convergence_path, "a") as output_file:
                output_file.write(f"Epoch {epoch}, {lr:.8f}, {loss:.8f}, {score:.6f}\n")
            # write to tensorboard
            tb_writer.add_scalar("Train/loss", loss, epoch)
            tb_writer.add_scalar("Train/lr", lr, epoch)
            tb_writer.add_scalar("Train/score", score, epoch)

def tbwrite_nlls(epoch=None, agent_nlls=None, prior_nlls=None):
    """ Writes the current epoch, loss, learning rate, and model score to CSV.
    """
    avg_agent_nll = torch.mean(agent_nlls)
    avg_prior_nll = torch.mean(prior_nlls)
    tb_writer.add_scalar("Train/agent_nll", avg_agent_nll, epoch)
    tb_writer.add_scalar("Train/prior_nll", avg_prior_nll, epoch)


def write_molecules(molecules, agent_lls, prior_lls, epoch):
    ...
    fraction_valid, validity_tensor, smiles = write_graphs_to_smi(smi_filename=smi_filename,
                                                                  molecular_graphs_list=molecules)

    # save the NLLs and validity status
    if epoch[-2:] != "-1":
        write_nlls(nll_filename=f"{smi_filename[:-3]}agent_nll",
                nlls=-agent_lls)
        write_nlls(nll_filename=f"{smi_filename[:-3]}prior_nll",
                nlls=-prior_lls)
...
def get_unique_tensor(smiles):

    unique_tensor = torch.ones(len(smiles), device="cuda")
    smiles_set = []

    for idx, smile in enumerate(smiles):
        if smile in smiles_set:
            unique_tensor[idx] = 0
        else:
            smiles_set.append(smile)
